...
class PollinationsPublicWrittenAnswerGradingProvider implements WrittenAnswerGradingProvider
{
    ...
    public function suggest(ExamSessionAnswer $answer): WrittenAnswerGradingSuggestion
    {
        ...
        if (! is_array($data)) {
            throw new RuntimeException('Pollinations public provider returned an invalid grading response.');
        }

        if (! array_key_exists('suggested_score', $data) || ! is_numeric($data['suggested_score'])) {
            throw new RuntimeException('Pollinations public provider returned a grading response without a numeric suggested_score.');
        }
        ...
        return new WrittenAnswerGradingSuggestion(
            ...
            rationale: trim((string) ($data['rationale'] ?? 'Suggested from the JSON evaluation request and student answer evidence.')),
    ...
    private function requestWithFallbacks(string $prompt): array
    {
        ...
        if (! $lastResponse instanceof Response) {
            throw new RuntimeException('Pollinations public grading request was not attempted.');
        }
    ...
    {
        $body = $response->body();
        $message = 'Pollinations public grading request failed: '.$body;

        if ($response->tooManyRequests()) {
            $message = 'Pollinations public free endpoint is busy/rate-limited right now. Please wait a minute and generate again, or configure GROQ_API_KEY/GOOGLE_GEMINI_API_KEY for reliable grading. Last response: '.$body;
        }

        if ($attempts !== []) {
            $message .= ' Attempts: '.implode('; ', $attempts);
        }
    ...
    {
        $note = 'Generated by the public Pollinations text endpoint because no Gemini or Groq key was configured. Use Groq/Gemini keys for private or production exams.';

        if ($usedInsecureTransport) {
            $note .= ' SSL verification was disabled for this public fallback because local PHP could not find a CA issuer certificate.';
        }

        if (count($attempts) > 1) {
            $note .= ' Public endpoint retries used: '.implode('; ', $attempts).'.';
        }
