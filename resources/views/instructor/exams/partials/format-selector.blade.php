...
<div class="grid gap-4 lg:grid-cols-3"
    x-data="{ selected: @js(old('display_format', $selected)) }"
    role="radiogroup"
    aria-label="{{ __('Exam display format') }}">
    @foreach ($formats as $key => $format)
        ...
        <label for="{{ $inputId }}"
            data-exam-format-option="{{ $key }}"
            x-bind:class="selected === @js($key) ? 'border-orange-400 ring-1 ring-orange-200' : 'border-slate-200 hover:border-orange-300'"
            ...
            <input id="{{ $inputId }}" type="radio" name="display_format" value="{{ $key }}"
                class="sr-only" x-model="selected" @checked($isSelected)>
            ...
            <div class="mt-4 flex flex-1 flex-col">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border"
                        x-bind:class="selected === @js($key) ? 'border-orange-500 bg-orange-500' : 'border-slate-300 bg-white group-hover:border-orange-400'">
                        <span x-show="selected === @js($key)" x-cloak class="h-2 w-2 rounded-full bg-white"></span>
                    </span>
                </div>
                ...
                <div class="mt-5 inline-flex items-center justify-center rounded-md border px-3 py-2 text-sm font-semibold transition"
                    x-bind:class="selected === @js($key) ? 'border-orange-500 bg-orange-50 text-orange-700' : 'border-slate-300 text-slate-700 group-hover:border-orange-300 group-hover:text-orange-700'">
                    <span data-exam-format-selected-label
                        x-text="selected === @js($key) ? @js(__('Selected format')) : @js(__('Choose this format'))">
                        {{ $isSelected ? __('Selected format') : __('Choose this format') }}
                    </span>
                </div>
            </div>
        </label>
